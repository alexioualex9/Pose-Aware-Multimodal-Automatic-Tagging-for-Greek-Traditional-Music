...
Run:
  Build:
    python main.py process_skeleton_sequences --model_name {STGCN} --set {train, val, test} --device {cpu, cuda, mps}
...
import argparse
from process_skeleton_sequences import build_npz
...
def create_parser():
    ...
    # ---- build ----
    c_e = sub.add_parser("process_skeleton_sequences", help="Build .npz clips with data [6,T,17] and write skeleton_index.json")
...
def main():
    ...
    if args.cmd == "process_skeleton_sequences":
        build_npz(args)
    elif args.cmd == "train":
        train(args)
    elif args.cmd == "eval":
